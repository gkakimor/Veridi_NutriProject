...
| [INVENTORY-PHYSICAL-COUNT-DISCOVERY-01](INVENTORY-PHYSICAL-COUNT-DISCOVERY-01.md) | Inventário Físico em sessões de inventário em lote (baseline INVENTORY-PHYSICAL-COUNT-PO-BASELINE-01, [BACKLOG §G](../BACKLOG.md)) | `EM_ANALISE` | 2026-09-15 | Recomendado: sessão `StockCount` (`INV-`) em contagem → revisão → encerrado/cancelado; saldo de referência por posição + saldo esperado congelado em cada registro de contagem, ajuste = diferença congelada aplicada no encerramento, sem bloquear movimentações; uma posição em uma sessão aberta; Contagem rápida mantida, gravando documento; sem tolerância; CSV controlado | D1 concorrência · D2 recontagem · D3 tolerância · D4 contar × aprovar bloqueiam; D5–D8 e P1–P7 com recomendação | NÃO IMPLEMENTADO. READY_TO_IMPLEMENT = NO. Baseline em 04d97ad (merge 9b011aa). Próxima: PO fecha D1–D4, depois INVENTORY-PHYSICAL-COUNT-01 |

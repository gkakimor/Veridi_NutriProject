...
|---|---|---|---|---|---|---|
| [DOCUMENT-TRANSITION-CONCURRENCY-DISCOVERY-01](DOCUMENT-TRANSITION-CONCURRENCY-DISCOVERY-01.md) | Transições de documento sob concorrência: cancelar e editar sem trava sobrescreviam a transição concorrente | `DECIDIDO` | 2026-09-19 | Cancelamentos e edições de Expedição, OC, Pedido, OP e Faturamento liam o status sem trava e gravavam depois do commit da outra transição. Reparo: travar a raiz, reler e decidir sobre o estado travado, FKs e CASCADE mantidos; conflito vira 409 sem retry. PO abriu a Fatia 1 só com os quatro P0 (R-S1, R-S2, R-O1, R-P1), sem migration. Entregue no chat e persistido na implementação | Os P1: Pedido (plano e reserva × cancelar; confirmar × cancelar), Faturamento, OC confirmar × cancelar e Lote | Parcial: Fatia 1 por DOCUMENT-TRANSITION-CONCURRENCY-01 em 2026-09-19 (§129, sem migration, na `main`, fora de PROD). Próxima: Fatia 2 (P1) |
| [INTERNAL-CONSUMPTION-REVERSAL-DISCOVERY-01](INTERNAL-CONSUMPTION-REVERSAL-DISCOVERY-01.md) | Estorno do Consumo Interno (CI-) lançado errado | `IMPLEMENTADO` | 2026-09-18 | P1–P4 decididas pelo PO com as recomendações: estorno próprio (A), tabela `internal_consumption_reversals` e entrada `INTERNAL_CONSUMPTION_REVERSAL`; ADMIN + QUALITY em lista própria; R-21 líquido na data do CI (R21-a); recusa com a posição em inventário aberto ou contada depois do CI. P5–P10 como recomendadas: parcial e vários, item inativo sim, sempre o mesmo lote, código `ECI-`, só consumo interno, sem estorno de estorno. Entregue só no chat e persistido na implementação | Nenhuma. Laterais registrados: INTERNAL-CONSUMPTION-BACKDATED-AFTER-COUNT-01 (fechado em 2026-09-19, §127) e DASHBOARD-INTERNAL-CONSUMPTION-01 | INTERNAL-CONSUMPTION-REVERSAL-01, entregue em 2026-09-18 (§126, migration `20260925093039`, na `main`, fora de PROD); o L2 por INTERNAL-CONSUMPTION-BACKDATED-AFTER-COUNT-01 em 2026-09-19 (§127, sem migration) |
| [MASTER-DATA-DELETE-ARCHIVE-DISCOVERY-01](MASTER-DATA-DELETE-ARCHIVE-DISCOVERY-01.md) | Cadastro mestre: quando excluir fisicamente, inativar ou arquivar (os nove cadastros da §114 e os auxiliares) | `IMPLEMENTADO` | 2026-09-17 | D1–D6 decididas pelo PO: exclusão física só por ADMIN, sem herdar a permissão de criar ou editar; qualquer uso, referência ou histórico real bloqueia — inclusive rascunho trabalhado, tarifa, oferta, referência de custo, proveniência, Json e código copiado —, e só o filho técnico nascido no mesmo ato e nunca usado (V1 DRAFT, PA do Produto, evento inicial da relação Item × Fornecedor) não conta; falha fechada; rastro append-only com motivo e retrato mínimo, sem contato nem segredo; Perfil de Produção ganha Arquivar/Desarquivar; nunca zero ADMIN ativo; FKs mantidas, com a segurança na aplicação (prévia, `FOR UPDATE`, recontagem, `pg_stat_xact_user_tables`). Entregue no chat e persistido no fechamento | Nenhuma bloqueante; quatro pontos de leitura para os handoffs (seção 12) | Fatia 0 implementada em 2026-09-17, sem migration: D5 em USER-LAST-ADMIN-GUARD-01 (§120) e D4 em PRODUCTION-PROFILE-ARCHIVE-01 (§121). Fatia 1 em 2026-09-18: MASTER-DATA-HARD-DELETE-01 (§125), com a migration da tabela do rastro, fechada de vez no mesmo dia por CUSTOMER-CNPJ-CREATION-HISTORY-MARKER-01 (marca `createdWithCustomerId`, migration aditiva `20260925093040`). Fatia 2 em 2026-09-19: MASTER-DATA-HARD-DELETE-02 (§128, código `f3b78060`), sem migration — Item, Produto + PA e Recurso industrial. As três fatias implementadas |
| [ITEM-DUPLICATE-SANITIZATION-DISCOVERY-01](ITEM-DUPLICATE-SANITIZATION-DISCOVERY-01.md) | Itens de matéria-prima e embalagem com o mesmo nome antes do nome único | `EM_ANALISE` | 2026-09-17 | 18 grupos, 39 Itens em ondas A/B/C. D2 decidida: duplicado sem uso é removido. D4 decidida: de-para no arquivo de decisão da carga e na documentação, sem alias e sem migration. Onda A (G1, G12, G14, G16, G17, G18) autorizada. D1 decidida ("A · B · C" com os nutrientes únicos) e Onda 2 aprovada (G2, G3, G5, G8, G9, G10, G15 e o par da sílica). Onda 3 decidida depois da revisão READ ONLY MASTER-DATA-DUPLICATE-REVIEW-03: G4 fundido, G7 e G13 renomeados, Modelo "X" excluído, G6 e G11 em revisão. Entregue no chat e persistido na implementação da Onda A | D3 (café verde) e o resto de D5 (potes); V1 e V4 com a Veridi para G6 e G11; Ondas A, 2 e 3 em PROD | Parcial: Onda A por ITEM-DUPLICATE-SANITIZATION-01 (§110), Onda 2 por MASTER-DATA-DUPLICATE-SANITIZATION-WAVE-2-01 (§118) e Onda 3 por MASTER-DATA-DUPLICATE-SANITIZATION-WAVE-3-01 (§124), 2026-09-17, aplicadas no `veridi_dev`, PROD intocado. Próximas: as ondas em PROD, G6 e G11 com a Veridi, a carga reproduzindo renomeação e consolidação, depois MASTER-DATA-NAME-UNIQUENESS-01 |

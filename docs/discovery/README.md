...
|---|---|---|---|---|---|---|
| [LABEL-ATTACHMENTS-ARCHITECTURE-DISCOVERY-01](LABEL-ATTACHMENTS-ARCHITECTURE-DISCOVERY-01.md) | Arquivo versionado do Item Rótulo e armazenamento de objetos (disco local × Cloudflare R2) | `IMPLEMENTADO` | 2026-09-16 | Cloudflare R2 aprovado; bucket privado `veridi-homologacao` criado pelo PO; abstração única `StorageAdapter` com `LOCAL_FS` e `R2`; versões imutáveis com vigente derivada, anular com motivo sem apagar bytes, restaurar como nova versão; PDF/PNG/JPEG até 25 MB por extensão, tipo e assinatura; enviar e restaurar Compras, Qualidade, Comercial e ADMIN; anular Qualidade e ADMIN. Decidido no chat e persistido na implementação, a partir do handoff | Configurar o R2 no Railway e autorizar o smoke real | LABEL-ATTACHMENTS-01, entregue em 2026-09-16 (na `main`, fora de PROD). Próximas: ativação do R2; ATTACHMENTS-R2-MIGRATION-01 |

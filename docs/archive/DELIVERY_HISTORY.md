# Histórico de deliveries — Veridi Nutrition

> **Arquivo histórico. Não precisa ser carregado para trabalho cotidiano.
> Consultar somente para investigar decisão/implementação anterior.**

Diário append-only das deliveries 01 a 40+, das auditorias e dos fechamentos de
bloco, preservado como estava em `PROJECT_STATE.md` antes da compactação
documental. Modelo de dados, decisões de implementação, testes e validação de
cada entrega.

Onde procurar em vez daqui:

- **Estado atual, release e próximo gate:** [PROJECT_STATE.md](../PROJECT_STATE.md)
- **Regras duráveis de negócio:** [PRODUCT_RULES.md](../PRODUCT_RULES.md)
- **Regras duráveis de UI:** [UI_BRAND.md](../UI_BRAND.md) — as "Durable UI
  decisions" repetidas abaixo têm ali a sua fonte única
- **Pendências abertas:** [BACKLOG.md](../BACKLOG.md) — inclui as decisões de
  produto não bloqueantes que este arquivo listava
- **Findings de auditoria:** [BACKLOG_HISTORY.md](BACKLOG_HISTORY.md)
...
# Estado do projeto antes da compactação documental

Texto que vivia no fim de `PROJECT_STATE.md` até a reorganização; a versão
condensada está no arquivo vivo.

...
| A regra de fechamento da OC não mudou com o widening: soma cheia e arredonda no fim, divergindo da Σ das linhas impressas | BACKLOG #18 permanece caracterizado e NÃO implementado | `apps/api/src/modules/purchase-orders/unit-price-precision.test.ts`, `packages/shared/src/purchase-orders.test.ts` |
...
| O fechamento `técnico → comercial` é uma operação nomeada, com o rounding mode do produto (`ROUND_HALF_UP`, nunca banker's), idempotente e em `Decimal` | um `.toFixed(4)` sem nome é indistinguível de defeito e acaba "corrigido" por engano | `apps/api/src/lib/commercial-price.test.ts` |

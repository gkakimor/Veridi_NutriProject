...
| Recusa de regra de negócio é 4xx com código e mensagem do domínio, nunca 5xx, e o texto que a tela mostra é o do domínio — não "Erro de validação" nem o genérico por status | FIX-05b — `CustomerMismatchError` escapava de `apply-fulfillment-plan` sem mapeamento e voltava 500 | `modules/customer-orders/cancelamento-op-cancelada.test.ts`, `web lib/erro-de-dominio-na-tela.test.ts` |

...
| **LISTS-EMPTY-ROW-390-RAW-01** | Linha de vazio escrita à mão (`td.table__empty` fora do `ListStatusRow` — seções e telas de detalhe, ex.: `SupplierItemsSection` com link na frase) não ganhou `.table__empty-body`: frase longa em tabela mais larga que a tela ainda pode passar da borda em 390px. Não medido tela a tela em LISTS-NAVIGATION-UX-WAVE-01 | UX | S |
...
| **API-INT-COERCION-REMAINING-01** | Três inteiros de ESCRITA ficaram fora de API-INT-COERCION-01 e seguem em `z.coerce.number().int()`: `capacityQuantitySchema` (`industrial-resources.schemas.ts`), `minutoDoDiaSchema` (`production-calendar.schemas.ts`) e `numberOfPartsSchema` (`production-orders.schemas.ts`). `"1e1"` grava 10 e `"0x10"` grava 16, dentro da faixa de cada um. Visto por leitura em API-PAGINATION-COERCION-01, sem teste. Troca por `inteiroDecimalSchema().pipe(...)` preservando mensagens, faixa e nulo | LOW | S |
| **INVENTORY-EXPORT-ONLY-WITH-STOCK-01** | `onlyWithStock` de `listInventoryQuerySchema` é `z.coerce.boolean()`, e `"false"` vira `true`. A tela de Estoque não manda o filtro desmarcado para a lista, mas o `ExportCsvButton` manda `filters={{ …, onlyWithStock }}` como `String(false)`, e `/inventory/export.csv` lê o mesmo schema: exportar com a caixa DESMARCADA sai só com estoque. Visto por leitura em API-PAGINATION-COERCION-01 (`z.coerce.boolean("false")` é `true`), não reproduzido pela rota. Mesmo `booleanFlag` dos relatórios resolveria | MEDIUM | XS |

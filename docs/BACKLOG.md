...
| **QUERY-BOOLEAN-PERMISSIVE-REMAINING-01** | Booleanos de query que ainda leem todo texto fora de `"true"` como `false`, calados: `onlyPending` e `onlyWithBalance` da fila da Qualidade (`?onlyPending=1` mostra a fila inteira), `active` dos Usuários (`?active=1` lista só os inativos), `archived` dos Modelos de Estrutura de Custos, das Políticas de preço (herdado por `.extend`) e dos Modelos de Formulação (`?archived=1` lista os não arquivados), e `includeArchived` dos anexos, lido cru de `request.query`, sem schema. As telas mandam `true`/`false` ou omitem. Achado pela varredura de QUERY-BOOLEAN-STRICTNESS-WAVE-02; a guarda (`lib/escalar-estrito-guarda.test.ts`) os mantém numa lista de dívida que reprova a entrada corrigida | LOW | XS |

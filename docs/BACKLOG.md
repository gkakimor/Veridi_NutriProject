...
| **SAVE-FEEDBACK-REMAINING-01** | Quatro telas fora do UX-ACTIONS-FEEDBACK-WAVE-02 dizem "Salvando…" e não confirmam o salvamento, e três delas (todas menos o Recebimento) dividem o `saving` com outras ações — o rótulo aparece no botão de salvar durante a ação vizinha: Faturamento (`BillingPage` "Salvar rascunho"), Ordem de Compra ("Salvar rascunho"/"Salvar previsão e observações"), Expedição ("Salvar separação") e Recebimento ("Salvar custo"). O caminho é o da onda: nome da ação em curso, `.form-status` só com a resposta, pendência da guarda que já existe. A Ordem de Produção saiu da lista em OP-SCHEDULE-STALE-ON-QUANTITY-01 (o botão de salvar sem pendência continua habilitado) | UX | S |

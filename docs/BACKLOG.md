...
| **SAVE-FEEDBACK-REMAINING-01** | Cinco telas fora do UX-ACTIONS-FEEDBACK-WAVE-02 dizem "Salvando…" e não confirmam o salvamento, e quatro delas (todas menos o Recebimento) dividem o `saving` com outras ações — o rótulo aparece no botão de salvar durante a ação vizinha: Faturamento (`BillingPage` "Salvar rascunho"), Ordem de Produção ("Salvar rascunho"/"Salvar observações"), Ordem de Compra ("Salvar rascunho"/"Salvar previsão e observações"), Expedição ("Salvar separação") e Recebimento ("Salvar custo"). O caminho é o da onda: nome da ação em curso, `.form-status` só com a resposta, pendência da guarda que já existe | UX | S |

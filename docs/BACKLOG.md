...
| **SAVE-ENABLED-NO-DIRTY-01** | Pedido e OC têm a pendência da guarda, mas "Salvar rascunho", "Salvar prazo e observações" e "Salvar previsão e observações" seguem clicáveis sem alteração — `UI_BRAND.md` pede desabilitado, e a OP já faz (SAVE-FEEDBACK-REMAINING-01). No documento novo o salvar também valida ("Selecione um fornecedor/cliente"): decidir se é a exceção da Formulação. Achado em SAVE-FEEDBACK-REMAINING-01, fora do escopo dela | UX | XS |
| **BILLING-SHIPMENT-UNSAVED-GUARD-01** | Faturamento (preços, referência externa, notas) e Expedição (quantidades da separação, notas) não registram a guarda de alterações não salvas: sair pelo menu ou pelo Voltar descarta o digitado sem perguntar. Achado em SAVE-FEEDBACK-REMAINING-01, que proibia infraestrutura de pendência nova | UX | S |
| **SAVE-THEN-COMMIT-STALE-01** | Emitir faturamento e confirmar expedição gravam o que está na tela e só então emitem/confirmam; se a segunda chamada é recusada, a primeira ficou gravada mas a tela não relê — no Faturamento o "Subtotal gravado" continua o de antes, na Expedição a tela segue com as linhas lidas antes do salvamento (que as recria no servidor). Nada falso é anunciado; o que falta é a releitura. Achado em SAVE-FEEDBACK-REMAINING-01 | LOW | XS |
...
| **W8** | Tetos de 100 que SELECTOR-CUTOFF-WAVE-01 classificou e não mexeu: dicas de homologação da OC (`PurchaseOrderPage.tsx:378`, relações do fornecedor), relações do cadastro de Item/Fornecedor (`SupplierItemsSection.tsx:29`), recurso industrial nos seletores de Estrutura de Custo e Roteiro e no `porId` do Planejamento (`CostTemplateDetailPage.tsx:207`, `ProductionProfileDetailPage.tsx:606`, `ProductionBoardPage.tsx:181`), amostras da ficha do Projeto (`ProjectDetailPage.tsx:114`), usuários (`UsersPage.tsx:41`, fora da fase — Auth) e FO-03 (`OperationalSheets.tsx:136`) | Seletor em contexto medido pequeno no dev: 29 recursos, até 59 relações por fornecedor, 1 amostra por projeto. Usuários e FO-03 são listagem, não seletor. Vira item da seção A quando algum passar do teto |

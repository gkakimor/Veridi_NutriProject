...
| **PREC-CMP-02** | Comparação de faixa ignora a unidade de medida | **RESOLVIDO / PUBLICADO em 2026-09-07.** A identidade da faixa passou a ser a quantidade FÍSICA normalizada na unidade do Item de produto acabado: `1 kg` e `1000 g` são a mesma faixa, `500 g` e `500 kg` são duas. Uma função canônica serve aplicação de política, criação manual e edição; a faixa nasce na unidade do produto e o template preserva a sua. Conversão oficial, `Decimal` de ponta a ponta, zero migration. Regra em [`PRODUCT_RULES.md`](PRODUCT_RULES.md) §68 |

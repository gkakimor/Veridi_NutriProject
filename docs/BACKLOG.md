...
| **PTBR-NUMERIC-DISPLAY-AUDIT-01** | Próxima capability depois de PTBR-NUMERIC-INPUT-ROLLOUT-01: auditar os valores numéricos SOMENTE LEITURA (tabelas, cards, dicas, PDFs) contra a regra pt-BR. Ponto de partida: `formatQuantity` não agrupa milhar (decisão para copiar de volta) e o campo agrupa fora do foco — "Aberto: 1234,5" ao lado de um campo que mostra "1.234,5"; e leituras cruas vistas no rollout: ficha do recurso industrial (`${powerKw} kW`, `R$ ${rateValue}` e a tabela de tarifas com ponto), Estrutura de Custos (`R$ ${rateValue}`, `${rateValue}%`, tarifa de referência), "Disponível não reservado" do consumo extra (`lotFreeQuantity` cru), "Disponível" do Plano de Atendimento (`finishedGoodsAvailable` cru) e a quantidade da linha do Orçamento fora de edição | UX | M |
| **ORDER-LINE-390-OVERLAP-01** | Pedido novo em 390px: o seletor de produto da linha (x 50–290) cobre o campo de quantidade (x 82–259), e o toque no campo cai no seletor — medido igual na main e no rollout (PTBR-NUMERIC-INPUT-ROLLOUT-01, smoke). Endurecimento mobile | UX | S |
...
| **LISTS-ERROR-FALSE-EMPTY-ADMIN-01** | Usuários e Documentos controlados (listas sem filtro nem página, carga única, fora das 16 de LISTS-LOADING-STALE-DATA-02): a falha da carga deixa "Nenhum usuário cadastrado." / "Nenhuma revisão cadastrada." junto do alerta — `!loading && lista.length === 0` não olha o erro. Por leitura de código; o remédio é o `ListStatusRow` | UX | XS |
| **LISTS-EMPTY-ROW-390-01** | Em 390px, a frase de vazio com ação na mesma célula (`td.table__empty`) passa da borda do contêiner rolável: Clientes com filtro, "Nenhum cliente encontrado para os filtros atuais. Limpar filtros", termina em x=437 com a borda em 378, e o botão fica sob a rolagem da tabela. Mesma medida na main antes de LISTS-LOADING-STALE-DATA-02 — é a célula de vazio sem quebra, como a dos Relatórios em SMALL-UX-CLEANUP-WAVE-01 | UX | S |
| **CONSULTATION-CUSTOMER-SWITCH-QUERY-01** | Visão do Cliente: trocar de cliente pela mesma rota (voltar/avançar do navegador entre dois clientes) pede a aba duas vezes — a aba consulta o cliente novo no render em que o shell ainda está montado, o shell a tira para carregar o resumo e ela consulta de novo ao voltar; a primeira resposta é descartada. Igual antes de LISTS-LOADING-STALE-DATA-02 (teste jsdom temporário: 2 consultas antes e depois) | LOW | XS |
| **LISTS-FILTER-INPUT-UX-01** | `DateRangeFilter` das listagens aplica a cada `change`: a data digitada no Chromium consulta os valores do meio (dia de um dígito, mês vazio, anos 0002/0020/0202 — ano curto volta 400) antes do dia completo, como os Relatórios faziam antes de REPORTS-SEARCH-UX-01 (`useFiltrosDigitados`). Desde LISTS-LOADING-STALE-DATA-01 cada consulta do meio mostra "Carregando…" e só a última vira a tela, mas as consultas continuam saindo. Separado de propósito da rodada de carregamento | UX | S |
...
| **TZ-LOCALE-STRING-REUSE-01** | `toLocaleString`/`toLocaleDateString` com opções (`{ timeZone }`) criam um formatador a cada chamada — o V8 só guarda o dele quando não há opções: ~54 µs por data, contra ~3–5 µs num formatador guardado. `instanteComercialPorExtenso` e `diaDoInstantePorExtenso` (shared), as datas do CSV (`api lib/csv.ts`, por célula), `diaComercialPorExtenso`, os extensos de `cost-source-selection`, `attention.service` e `quote-price-origin`, e `web lib/dates.ts`. Volume por requisição não medido; a troca por formatador guardado tem de repetir as opções padrão do `toLocale*` e o `"Invalid Date"` de data inválida, que o `format` recusa com `RangeError`. Achado em TZ-FORMATTER-REUSE-01, fora do escopo dela | LOW | S |
...
| **ROUTE-DRAFT-SAVE-INFLIGHT-EDIT-01** | Roteiro de Produção: o que se digita no rascunho enquanto "Salvar rascunho" grava é trocado pelo gravado na releitura, sem pendência — os campos seguem editáveis durante a gravação, e a releitura depois de salvar o rascunho traz o servidor normalizado por regra (teste jsdom temporário em ROUTE-IDENTIFICATION-SAVE-DRAFT-01: 700 digitado durante a gravação de 500 voltou 500, sem "Alterações não salvas") | UX | S |
| **CONTEXT-ORIGIN-LABEL-ROUTE-01** | O cadastro de recurso aberto pelo Roteiro de Produção diz "← Voltar para tela anterior": `rotuloDaOrigem` (`lib/use-contextual-create.ts`) não conhece `/planejamento/perfis-producao`. Visto no smoke de ROUTE-CONTEXT-RESTORE-01 | UX | XS |
...
| **REPORTS-PRINT-UNACCEPTED-FILTER-01** | "Filtros aplicados" do PDF dos relatórios lista toda chave da URL, inclusive a que o schema DAQUELE relatório não aceita e a API descarta: `/print/relatorios/R-08?customerId=…` sai "Cliente: CLI-… · Razão social" sobre o recorte inteiro; chave que nenhum schema conhece sai como veio (`foo: bar`). Nenhuma tela manda — só URL digitada. Desde REPORTS-PRESENTATION-WAVE-02 nenhum id sai cru; o que falta é cada definição de `REPORT_PRINT_DEFINITIONS` declarar as chaves do seu schema (o teste "fornecedor e cliente juntos" de WAVE-01 fixa o comportamento de hoje) | UX | S |
| **FO03-ROW-SITUATION-01** | FO-03 descreve mal duas pendências do próprio recorte (`OperationalSheetsPdf.tsx`, `QualityPendingPdf`): a coluna Qualidade é `LOT_STATUS_LABELS[lotStatus]` e ignora `isExpired` — no smoke de FO03-PENDING-CUTOFF-01 a pendência com validade 31/01/2026 saiu "Aguardando liberação", enquanto Documentos / CoA diz "Vencido" e FO-01/FO-02 usam `situacaoDoLote`; e a Pendência do laudo rejeitado sai "Aguardando liberação" (o `else` de `pendenciaDoLote`, escrito quando a folha trazia todos os lotes), com o lote bloqueado. O primeiro é uma linha; o texto do segundo é decisão do PO. Anterior à rodada, que não mudou o documento | UX | XS |
...
| **API-PAGINATION-COERCION-01** | `page`/`pageSize` das listagens (28 schemas de consulta, `projects.schemas.ts` incluído) seguem em `z.coerce.number()`: `?page=1e1` é a página 10 e `?pageSize=0x10` são 16 linhas. Só leitura, dentro de min/max — mantido fora de INPUT-DATE-CONTRACT-WAVE-01, que tratou inteiro de escrita. Trocar por `inteiroDecimalSchema()` é mecânico | LOW | S |

...
| ID | Título | Sev. | Tam. |
|---|---|---|---|
| **API-ERROR-HANDLER-RECURSION-01** | O `setErrorHandler` global de `app.ts` (desde `f3a4c666`, também em `release/prod`) delega para `app.errorHandler`, que no Fastify 5 devolve o próprio handler: todo erro sem tradução recursa até `RangeError: Maximum call stack size exceeded`, a resposta é 500 com essa frase e o log perde o erro original. Visto nos testes vermelhos da rodada; posto em sessão separada no mesmo dia | HIGH | XS |

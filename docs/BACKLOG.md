...
| **ROUTE-CONTEXT-RESTORE-01** | Roteiro de Produção: na volta de um cadastro no contexto, `restoreDraft` devolve etapas, quantidade e unidade de referência, e a leitura do roteiro, que chega depois, sobrescreve os três com o gravado — nome e descrição sobrevivem porque o `load()` os compara com a leitura anterior. Visto por teste jsdom em SELECTOR-CUTOFF-WAVE-02 (etapa restaurada "Etapa Restaurada" voltou "Encapsulamento"). Hoje a única saída é "Cadastrar recurso" do catálogo vazio: perde-se a edição de etapas feita antes | UX | S |

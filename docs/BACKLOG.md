...
| **PREC-CMP-02** | Comparação de faixa ignora a unidade de medida | **ABERTO — aguarda PO.** A mesma comparação casa apenas quantidade: `1 kg` e `1000 g` são a mesma faixa física e seriam tratadas como duas, e duas faixas de `500` em unidades diferentes como uma. O contrato só garante compatibilidade com a unidade do produto acabado, não igualdade. Achado do PREC-CMP-01; converter antes de comparar é decisão de domínio |

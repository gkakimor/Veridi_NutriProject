...
| **REPORTS-QUERY-BOOLEAN-PERMISSIVE-01** | `booleanFlag` e `all` de `reports.schemas.ts` leem todo texto fora de `false`/`0`/`no`/vazio como `true`: `?all=abc` ou `?all=off` pede o relatório completo. Não é `z.coerce.boolean()` (o `"false"` funciona), por isso ficou fora de API-STRICT-SCALAR-CONTRACT-WAVE-01. Migrar para `booleanoDeConsultaSchema` exige conferir se alguma tela manda `1`/`0` | LOW | XS |
| **CUSTOMER-MATERIALS-ONLY-WITH-BALANCE-PERMISSIVE-01** | `onlyWithBalance` de `listCustomerMaterialsQuerySchema` lê todo texto fora de `"true"` como `false`, em silêncio: `?onlyWithBalance=1` lista também o material sem saldo. Visto por leitura em API-STRICT-SCALAR-CONTRACT-WAVE-01 | LOW | XS |

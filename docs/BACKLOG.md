...
| 9t | P1 | **PRODUCTION-PROFILE-ARCHIVE-01** — arquivar e desarquivar o Perfil de Produção | **Liberado pelo PO em 2026-09-17** · Fatia 0 de [MASTER-DATA-DELETE-ARCHIVE-DISCOVERY-01](discovery/MASTER-DATA-DELETE-ARCHIVE-DISCOVERY-01.md), D4: ADMIN e Produção arquivam e desarquivam; arquivado sai da lista padrão e dos seletores e não entra em compromisso novo (padrão do Produto, aplicação à OP, aplicação automática na criação da OP); histórico e cópias nas OPs seguem consultáveis · a coluna `archivedAt` já existe, sem rota nem tela · **sem migration** | Implementar | — |
| 9u | P1 | **USER-LAST-ADMIN-GUARD-01** — nunca zero ADMIN ativo | **Liberado pelo PO em 2026-09-17** · Fatia 0 do [discovery](discovery/MASTER-DATA-DELETE-ARCHIVE-DISCOVERY-01.md), D5: recusar a inativação e o rebaixamento do último ADMIN ativo; ninguém inativa a si mesmo nem retira de si o papel ADMIN — outro ADMIN executa · contagem e gravação na mesma transação, sob trava · **sem migration** | Implementar | — |
| 9v | P1 | **MASTER-DATA-HARD-DELETE-01** — exclusão física do cadastro errado e nunca usado: infraestrutura, Fornecedor, Cliente, Modelos e Perfil de Produção | **Liberado pelo PO em 2026-09-17** · Fatia 1 do [discovery](discovery/MASTER-DATA-DELETE-ARCHIVE-DISCOVERY-01.md), D1–D3 e D6: só ADMIN; qualquer uso, referência ou histórico real bloqueia, com falha fechada (só o filho técnico nascido no mesmo ato e nunca usado não conta); prévia `deletion-check`, transação com `FOR UPDATE`, recontagem e `pg_stat_xact_user_tables`, com rollback em efeito inesperado; rastro append-only com motivo obrigatório, executor, data e retrato mínimo, sem contato nem segredo · FKs intocadas · **migration aditiva** (só a tabela do rastro) | Implementar | 9t, para o Perfil de Produção |
| 9w | P1 | **MASTER-DATA-HARD-DELETE-02** — exclusão física de Item, Produto + PA e Recurso industrial | **Liberado pelo PO em 2026-09-17** · Fatia 2 do [discovery](discovery/MASTER-DATA-DELETE-ARCHIVE-DISCOVERY-01.md), mesmas decisões, nos cadastros com CASCADE no caminho (movimento de estoque, referência de custo, contagem, custo e precificação, tarifa) · o PA sai só junto do Produto e sem uso próprio · **sem migration** (usa o rastro da Fatia 1) | Implementar depois da Fatia 1 | 9v |

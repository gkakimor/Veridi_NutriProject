...
| **PTBR-NUMERIC-INPUT-ROLLOUT-01** | Próxima capability depois de PTBR-NUMERIC-INPUT-FOUNDATION-01 (2026-09-13, [`PROJECT_STATE.md`](PROJECT_STATE.md), seção própria; regra em [`UI_BRAND.md`](UI_BRAND.md), "Campos numéricos e valores pt-BR"). Migrar os campos realmente numéricos para `IntegerField`/`DecimalField`/`MoneyField`/`PercentField`, com a leitura de `parsePtBrNumber` na borda: ~38 telas e componentes com `inputMode="decimal"`, `exigirDecimal`/`parseDecimalInput` ou `lerInteiroOpcional` (Pedido, OC, Produto, Formulação, Custos, Precificação, Orçamento, Faturamento, Recebimento, Estoque, Planejamento…) e os três `type="number"` (capacidade do recurso industrial, na ficha e no cadastro; partes da OP). CEP de Cliente e Fornecedor fica fora — é identificador. No fim: esvaziar a lista de `campo-numerico-guarda.test.ts` (vira proibição de `type="number"`) e aposentar `parseDecimalInput`/`AJUDA_DECIMAL` na web. Decidir tela a tela: `scale` do domínio de cada campo; `1.234` digitado passa de 1,234 (parser de hoje) a recusado como ambíguo; `formatQuantity` não agrupa milhar e o campo agrupa fora do foco; teclado `decimal` do iOS não tem sinal de menos (campo com `allowNegative` pode pedir `inputMode="text"`); o campo não limita os dígitos da parte inteira — conferir a resposta da API ao estouro da coluna. QUOTE-PERCENT-FIELDS-01 e FORMULATION-DOSES-INPUT-01 tocam os mesmos campos: fechar junto ou dizer por que não | UX | L |

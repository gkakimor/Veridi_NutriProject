...
| **LISTS-LOADING-STALE-DATA-02** | O que LISTS-LOADING-STALE-DATA-01 não cobriu, por leitura de código. As outras 16 listas paginadas guardam linhas, total e erro em `useState` soltos, com `setLoading`/`then` sem guarda de ordem — filtro novo deixa o recorte anterior à vista, a última resposta a chegar vira a tela e a falha aparece junto das linhas de antes: Clientes, Fornecedores, Itens de estoque, Produtos Acabados, Item × Fornecedor, Formulações, Modelos de Formulação, Recursos industriais, Modelos de Estrutura de Custo, Políticas de Precificação, Precificação, Roteiros de Produção, Estoque, Movimentações, Materiais de Clientes e a busca da Visão do Cliente. `useScopedList` (abas da Visão do Cliente) tem a guarda, mas mantém as linhas do cliente anterior durante a carga e volta à página 1 por efeito (duas consultas fora da primeira página). Quadro e Calendário de Produção guardam o quadro anterior sob o "Carregando…", sem guarda de ordem. Remédio pronto: `useListQuery`/`ListStatusRow` (e `useFilteredPage` onde a página é estado) | UX | M |
...
| **ROUTE-IDENTIFICATION-SAVE-DRAFT-01** | Roteiro de Produção: "Salvar identificação" com rascunho pendente recarrega o roteiro e troca base, unidade e etapas digitadas pelo gravado, sem aviso — a pendência some junto (teste jsdom temporário em ROUTE-CONTEXT-RESTORE-01: base 500 voltou 1000, etapa digitada voltou "Encapsulamento"). É o inverso do que o Modelo de Estrutura e o Template de Formulação corrigiram com a leitura anterior por campo; aqui o bloco do rascunho não pode usar a mesma comparação depois de "Salvar rascunho", porque o servidor normaliza o que grava (`250,5` → `250.5`) | UX | S |
...
| **COST-USAGE-RESOURCE-BYID-01** | Estrutura de Custos: o recurso da linha criado no contexto (ou restaurado do rascunho) fora dos 50 primeiros ativos volta com o campo vazio e o id escolhido por baixo — `resources` só conhece a primeira página e o que a busca achou; o controle dentro dos 50 volta preenchido (teste jsdom em SELECTOR-CUTOFF-WAVE-02). Por leitura de código, o aviso de energia fora do modo direto (`onCreated`) lê a mesma lista e não dispara para esse recurso. Remédio pronto: `useRecursosDoSeletor`, com resolução pelo id depois da página; fora da wave, que só trocou a tarifa do kWh derivado | LOW | S |

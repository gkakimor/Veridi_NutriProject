...
2. **O que se prova, pela interface.** Pré-condição pode nascer por API
   (`fixtures/cadastros.mjs`, `fixtures/comercial.mjs`, `fixtures/producao.mjs`)
   quando não é o comportamento sob teste, economiza muito tempo, a rota já tem
   teste de API e não pula a pré-condição do cenário. GET de conferência é livre;
   Prisma e SQL direto, nunca.
...
| `orcamentos-hub-e-pagina-da-versao.mjs` | Comercial → Orçamentos sobre 24 versões carimbadas por API: menu, padrão Em aberto (consulta Rascunho + Enviado, fora da URL), Todos, busca numa consulta depois da pausa, cliente e projeto pelo id, período personalizado em dia civil e paginação; linha, Enter e "Abrir" abrem a página da versão com `?voltar=`, e "← Voltar para Orçamentos" restaura busca, status, cliente, projeto, período e página — na URL e na tela; menu ativo e título da aba na lista e na versão; recorte sem resultado com Limpar filtros; 500 com Tentar novamente; 404; e 390px sem rolagem lateral, com o "Abrir" da coluna fixa clicável (QUOTES-HUB-01, QUOTE-WORKSPACE-NAVIGATION-01, E2E-BASELINE-REDESIGN-WAVE-03) |
| `projeto-aprovado-vende-de-novo.mjs` | um Projeto da execução vende duas vezes pelo fluxo atual: V1 criada pela ficha, enviada e aceita na página da versão — o Fechamento diz que falta aprovar —, aprovação na ficha e Pedido 1 no Fechamento; JÁ APROVADO, "Novo orçamento" dá a V2 renegociada e aceita, a ficha lista a V1 ainda aceita com o Pedido 1, e o Pedido 2 nasce no Fechamento da V2, cada Pedido com a sua versão de origem; a V3 enviada vencida diz "Vencido", explica e não aceita (COM-CORE, E2E-BASELINE-REDESIGN-WAVE-03) |
...
| `resumo-comercial-do-projeto.mjs` | a coluna Comercial da ficha do Projeto, LEITURA do que se edita e envia na página da versão: vazia e legível sem proposta; com a V1 em rascunho, valor e itens e NÃO data de envio; enviada, a data; criada a V2 em rascunho, o último orçamento é a V2 e a última proposta enviada continua a V1, em linha própria — o passo que impede colar o rótulo de uma versão na data de outra; a condição parcelada gravada na V2, por extenso; e o bloco em 390px (PROJECT-COMMERCIAL-SUMMARY-01, §82, E2E-BASELINE-REDESIGN-WAVE-03) |
| `condicoes-do-orcamento-sobrevivem-a-linha.mjs` | na página própria da versão, condições digitadas e NÃO salvas — validade, desconto, observação, forma de pagamento e parcelas — continuam nos campos depois de adicionar produto, editar quantidade e preço, remover linha e de uma edição de linha recusada (`1.234`, sem gravar), com "Alterações não salvas" e salvar habilitado; uma segunda aba prova que nada foi gravado; salvar grava e reabrir mostra o gravado; com a V2 criada pela ficha e condição suja nela, ir para a V1 passa pela guarda de saída (decisão E): "Continuar editando" fica na V2 sem gravar, "Sair sem salvar" abre a V1 com as condições DELA, em leitura, e a V2 volta com o gravado (QUOTE-DRAFT-STATE-01, E2E-BASELINE-REDESIGN-WAVE-03) |
| `envio-exige-condicoes-e-linhas-salvas.mjs` | as duas suítes do envio fundidas (decisão D), na página da versão: A. validade alterada e não salva prende "Enviar ao cliente" com a mensagem das condições ligada ao botão, insistir não abre confirmação nem manda envio, adicionar e remover produto não liberam, salvar libera; B. preço novo prende antes de sair do campo, o salvamento cai (503 provocado, uma vez, declarado) — campo com o preço novo, erro na tela, servidor com o antigo, mensagem dos produtos no botão, insistir não envia — e a nova tentativa grava sem recarregar; C. envio confirmado, uma requisição só; E. enviada, a versão reaberta é leitura, com a validade e o preço novos, e a ficha lista a validade nova; D. com a V2 existindo, o PDF da V1 traz a V1 e o que foi congelado, e o da V2 traz a V2 (QUOTE-SEND-DIRTY-01, QUOTE-SEND-LINE-DRAFT-01, E2E-BASELINE-REDESIGN-WAVE-03) |
| `preco-herdado-sobrevive-ao-tab.mjs` | pelo fluxo real — V1 enviada e aceita na página da versão, ficha, "Criar nova versão" (não "Duplicar como nova versão", que é outro contrato) —, a linha da V2 nasce com o preço herdado, ligada ao acordo; Tab pela quantidade, pela unidade e pelo preço não manda atualização de linha; reaberta, preço, origem e vínculo continuam e o envio segue disponível; mudar a quantidade de verdade solta o preço herdado (QUOTE-LINE-NOOP-BLUR-01, E2E-BASELINE-REDESIGN-WAVE-03) |
| `prazo-invalido-nao-apaga.mjs` | na página da versão, com prazo 30 e 3 parcelas gravados: letra no prazo não entra e nada muda; prazo `0` mostra o erro no próprio campo, ligado por `aria-describedby`, mantém o texto, diz "Alterações não salvas", prende "Salvar condições" e "Enviar ao cliente", e nenhuma gravação sai do navegador; uma segunda aba prova o 30 gravado; vírgula nas parcelas não entra e 121 é recusado no campo; corrigidos, prazo e parcelas salvam como inteiros e a versão reaberta mostra 45 e 4; console limpo (QUOTE-INT-FIELDS-01, E2E-BASELINE-REDESIGN-WAVE-03) |
...
| `formacao-de-preco-do-novo-orcamento.mjs` | a recompra forma o preço clicando, com cliente da execução: o acordo nasce (versão → aceite → ficha → aprovar → versão → Fechamento → Pedido); condição vigente na mesma quantidade nasce com o preço acordado e a origem visível até o Pedido; quantidade diferente solta o preço, diz para qual quantidade a condição foi negociada, e manter exige motivo (409 provocado e declarado); reajuste de 8% com prévia e o valor fechado pelo servidor (COM-PRICE, §74, E2E-BASELINE-REDESIGN-WAVE-03) |
...
Hoje: 11 suítes na bateria `wave-01-02`, 2 provas e 8 na `wave-03` (o Orçamento na
página própria da versão e o Hub de Orçamentos); 8 que ainda leem código fixo, saldo
ou primeiro registro (grupo C, E2E-CORPUS-MASS-01) — `desconto-do-pedido-chega-ao-faturamento`
também pelo fluxo antigo do Orçamento; 2 a reescrever — o golden path, que para no
Pedido pelo fluxo antigo, e a de `PROD-000214`.
...
| `ui.mjs` | `esperarRota` pelo pathname (consulta como `?ids=` não quebra), `escolherOpcao` ignorando "+ Novo…", `abrirPeloMenu` pelo grupo e item do menu lateral |
...
| `comercial.mjs` | `criarProjeto`, `criarProdutoDoProjeto`, `criarVersao`, `adicionarLinha` e `precificarLinha` (a linha é achada pelo produto, nunca pela posição), `gravarCondicoes`, `enviarVersao`, `recusarVersao`, `lerVersaoDoServidor` — por API, carimbados, código e rótulo `ORC-… · Vn` lidos da resposta |
| `comercial-ui.mjs` | os gestos do Orçamento na tela: `abrirVersao` e `esperarVersaoNaTela`, `voltarAoProjeto`, `criarNovaVersao` (com o botão da ficha dito pela suíte), `aprovarProjeto`, `enviarAoCliente`, `registrarAceite`, `gerarPedido` e `aguardarReleitura` — esperam a tela e devolvem id da rota, URL e rótulo |
...
lote, pedido, preço ou finding específico da base. Fixture de massa
(`cadastros`, `comercial`, `producao`) cria massa genérica carimbada, não navega e
não afirma regra: prepara. Gesto de tela (`ui`, `comercial-ui`) navega e espera a
tela terminar, e também não afirma regra — quem confere é a suíte. Se a suíte
precisa de um registro específico, ele pertence à suíte. Contrato das fixtures em
`fixtures/fixtures.test.ts`; guardas do runner e das suítes em
`scripts/e2e-run.test.ts`.

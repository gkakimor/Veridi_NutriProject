...
Nada aqui "corrige" o legado: o que não dá para afirmar fica vazio e vira
pendência para a Veridi decidir.
"""
...
def _registrar_fontes(pac: Pacote, fontes: dict, pesquisa_nova: Path | None) -> None:
    usos = {
        ...
        "precos_fornecedores.csv": "Relação item × fornecedor e ofertas do legado (07); itens só com preço",
    ...
    for numero_linha, linha in enumerate(fontes["clientes.csv"], start=2):
        ...
        if not razao:
            pac.pendencia(
                ...
                "Informar a razão social. Se o cliente não tiver outra, repetir o nome fantasia.", True,
            )
        if cnpj:
        ...
        if endereco and partes["revisar"]:
            pac.pendencia(
                ...
                "Conferir LOGRADOURO, NUMERO, COMPLEMENTO e BAIRRO a partir de ENDERECO_ORIGINAL; "
                "o que não for possível afirmar pode ficar vazio.", False,
            )
...
def _fornecedores(pac: Pacote, fontes: dict) -> None:
    ...
    for numero_linha, nome in enumerate(nomes, start=2):
        ...
        compacto = R.normalizar_nome(nome).replace(" ", "")
        if len(compacto) <= 3:
            ...
            if len(compacto) <= 2 or candidatos:
                ...
                pac.pendencia(
                    ...
                    + f". Uso: {uso(nome)}.",
                    "Informar o nome completo (razão social) ou, se for repetição de outro fornecedor, "
                    "marcar NAO_IMPORTAR.",
                    False,
                )
        normalizado = R.normalizar_nome(nome)
        if normalizado == "MERCADO LIVRE":
            pac.pendencia(
...
def _itens(pac: Pacote, fontes: dict) -> None:
    ...
    def uso(codigo: str) -> str:
        ...
        if precos_por_item.get(codigo):
            fornecedores = sorted({R.limpar(l["fornecedor"]) for l in precos_por_item[codigo]})
            partes.append(f"{len(precos_por_item[codigo])} preço(s) de fornecedor ({', '.join(fornecedores[:4])}{' …' if len(fornecedores) > 4 else ''})")
    ...
    for codigo in extras:
        e = enriquecimento.get(codigo, {})
        nomes_preco = Counter(R.limpar(l["materia_prima_fonte"]) for l in precos_por_item.get(codigo, []) if R.limpar(l["materia_prima_fonte"]))
        nutrientes_preco = Counter(R.limpar(l["nutriente"], zero_e_vazio=True) for l in precos_por_item.get(codigo, []) if R.limpar(l["nutriente"], zero_e_vazio=True))
    ...
    por_descricao: dict[str, list[dict]] = defaultdict(list)
    for c in candidatos:
        tipo = c["tipo"] or "MATERIA_PRIMA"
        arquivo = MATERIAS_PRIMAS if tipo == "MATERIA_PRIMA" else EMBALAGENS
        ...
        registro = {
            ...
            "DESCRICAO": c["nome"],
            "NUTRIENTE_DECLARADO": c["nutriente"] if tipo == "MATERIA_PRIMA" else "",
            "TIPO_ITEM": tipo,
            "UNIDADE_ESTOQUE": "kg" if tipo == "MATERIA_PRIMA" else "un",
            "FAMILIA": familia if tipo == "MATERIA_PRIMA" else "",
            "PUREZA_PADRAO": pureza if tipo == "MATERIA_PRIMA" else None,
            "FONTE_DO_REGISTRO": FONTE_DO_REGISTRO[c["origem"]],
            "STATUS_REVISAO": "",
            "OBSERVACAO_REVISAO": "",
            "_arquivo": arquivo,
            "_origem": c["origem"],
            ...
            "_nome": c["nome"],
            "_codigo": c["codigo"],
            "_ofertas": len(precos_por_item.get(c["codigo"], [])),
        }
        pac.itens.append(registro)
        nome = c["nome"] or f"(sem descrição) cód. {c['codigo']}"
        if c["nome"]:
            por_descricao[R.normalizar_nome(c["nome"])].append(registro)

        if not c["nome"]:
            pac.pendencia(
                arquivo, chave, nome, "CAMPO_OBRIGATORIO_AUSENTE", "Descrição vazia em todas as fontes.",
                "Informar a descrição do item ou marcar NAO_IMPORTAR.", True,
            )
        if c["origem"] != "CADASTRO":
            pac.pendencia(
                ...
                "Confirmar se o item existe e deve ser cadastrado (mudar STATUS_REVISAO para OK) ou marcar "
                "NAO_IMPORTAR. Conferir descrição, tipo e unidade.",
                True,
            )
        else:
            motivos = []
            ...
            if tipo == "EMBALAGEM" and familia_norm not in ("", "EMBALAGEM", "EMBALAGENS"):
                motivos.append(f"tipo EMBALAGEM, mas a família no CMV é \"{c['familia_legado']}\"")
            if tipo == "MATERIA_PRIMA" and familia_norm in ("EMBALAGEM", "EMBALAGENS"):
            ...
            if motivos:
                pac.pendencia(
                    ...
                    "; ".join(motivos) + f". Tipo e unidade atuais: {tipo} / {registro['UNIDADE_ESTOQUE']}.",
                    "Confirmar TIPO_ITEM e UNIDADE_ESTOQUE (o tipo informado vale, mesmo que a linha fique "
                    "nesta planilha) e mudar STATUS_REVISAO para OK.",
                    True,
                )

        if c["familia_legado"] and (
            (tipo == "MATERIA_PRIMA" and not direta)
            or (tipo == "EMBALAGEM" and R.normalizar_nome(c["familia_legado"]) not in ("EMBALAGEM", "EMBALAGENS"))
        ):
            pac.legado.append(
                Legado(
                    arquivo, chave, "familia (CMV)", c["familia_legado"], "itens_enriquecimento.csv",
                    "O ERP não tem família equivalente"
                    + (" — o item entra como OUTRA_MATERIA_PRIMA" if tipo == "MATERIA_PRIMA" else " a um item de embalagem")
    ...
    for descricao, grupo in por_descricao.items():
        if len(grupo) < 2:
            continue
        for registro in grupo:
            ...
            pac.pendencia(
                ...
                f"Mesma descrição de: {outros}. Este: nutriente {registro['NUTRIENTE_DECLARADO'] or '—'}; "
                ...
                "Se forem itens diferentes (fonte, grau, fornecedor), diferenciar a DESCRICAO.",
                False,
            )
...
def _produtos(pac: Pacote, fontes: dict) -> None:
    ...
    for codigo in sorted(formulas):
        ...
        registro = {
            ...
            "CLIENTE": (cliente["NOME_FANTASIA"] or cliente["RAZAO_SOCIAL"]) if cliente else "",
            "STATUS_REVISAO": "",
            "OBSERVACAO_REVISAO": "",
            "_nome": nome,
            "_fonte": (
        ...
        if len(clientes_legado) > 1:
            pac.pendencia(
                ...
                "Informar em CHAVE_CLIENTE o cliente dono do produto (ou deixar vazio se for produto sem cliente).",
                True,
            )
            ...
            pac.pendencia(
                ...
                "Informar a CHAVE_CLIENTE correta ou deixar vazio.", True,
            )
    ...
    for linha in fontes["cmv_produtos.csv"]:
        ...
        pac.legado.append(
            Legado(
                ...
                "exatamente com o cadastro; custeio (Bloco G) fica fora desta carga.",
            )
        )
...
def _ofertas(pac: Pacote, fontes: dict) -> None:
    ...
    for numero_linha, linha in enumerate(fontes["precos_fornecedores.csv"], start=2):
        ...
        registro = {
            ...
            "ITEM": item["DESCRICAO"] if item else R.limpar(linha["materia_prima_fonte"]),
            "TIPO_ITEM": item["TIPO_ITEM"] if item else "",
            "UNIDADE_ESTOQUE": item["UNIDADE_ESTOQUE"] if item else "",
            "PRECO_BRL": preco,
            "MOEDA": "BRL",
            "UNIDADE_DO_PRECO": "kg",
            "PEDIDO_MINIMO_PLANILHA": pedido_bruto,
            "HOMOLOGADO_NO_LEGADO": "SIM" if afirmativo(linha["homologado"]) else "",
            "TIPO_FONTE": "OFERTA_FORNECEDOR_LEGADO",
            "STATUS_REVISAO": "",
            "OBSERVACAO_REVISAO": "",
            "_nome": f"{item['DESCRICAO'] if item else codigo} × {fornecedor_nome}",
        ...
        if pedido_bruto and interpretar_pedido_minimo(pedido_bruto) is None:
            pac.pendencia(
                OFERTAS, chave, nome, "PEDIDO_MINIMO_AMBIGUO",
                f"Pedido mínimo \"{pedido_bruto}\" não é número + unidade reconhecível.",
                "Reescrever como número e unidade (ex.: 1000un, 25kg) ou deixar vazio.", False,
            )
        pac.legado.append(
            Legado(
...
def _precos_mercado(pac: Pacote, fontes: dict, pesquisa_nova: Path | None) -> None:
    ...
    for item in sorted(pac.itens, key=lambda i: int(i["_codigo"])):
        ...
        ofertas_txt = (
            f" O item tem {item['_ofertas']} oferta(s) de fornecedor do legado — ver 07."
            if item["_ofertas"] else ""
        )
        if achados.get(codigo):
            for achado in achados[codigo]:
                unidade_ref = R.unidade_do_catalogo(achado["unidade_bruta"])
                normalizado = (
                    R.normalizar_preco(achado["preco"], achado["quantidade"], unidade_ref, item["UNIDADE_ESTOQUE"])
                    if unidade_ref else None
                )
                if normalizado is not None:
                    memoria = R.memoria_calculo(
                        achado["preco"], achado["quantidade"], unidade_ref, item["UNIDADE_ESTOQUE"], normalizado
                    )
                    status = "A_REVISAR"
                    anterior = achado["canonico_anterior"]
                    if anterior is not None and achado["uom_anterior"] == item["UNIDADE_ESTOQUE"]:
                    ...
                    memoria = (
                        ...
                        f"{item['UNIDADE_ESTOQUE']} (dimensões diferentes ou unidade fora do catálogo)."
                    )
                    status = "NAO_NORMALIZAVEL"
                qtd_estoque = (
                    R.quantidade_em_unidade_estoque(achado["quantidade"], unidade_ref, item["UNIDADE_ESTOQUE"])
                    if unidade_ref else None
                )
                ...
                chave = R.chave_referencia(codigo, achado["url"], str(achado["preco"]), str(achado["quantidade"]), achado["unidade_bruta"])
                pac.precos.append(
                    {
                        "CHAVE_MIGRACAO": chave,
                        **base,
                ...
                if status == "NAO_NORMALIZAVEL":
                    pac.pendencia(
                        PRECOS, chave, item["DESCRICAO"], "UNIDADE_INVALIDA",
                        f"{item['CHAVE_MIGRACAO']}: {memoria}",
                        "Usar o preço publicado só como ordem de grandeza, ou revisar a unidade do item em 03/04.",
                        False,
                    )
            continue

        personalizada = item["TIPO_ITEM"] == "EMBALAGEM" and R.PADRAO_PERSONALIZADA.search(item["DESCRICAO"])
        if sem_resultado.get(codigo):
        ...
        chave = R.chave_referencia(codigo, status)
        pac.precos.append(
            {
                "CHAVE_MIGRACAO": chave,
                **base,
        ...
        pac.pendencia(
            PRECOS, chave, item["DESCRICAO"], "PRECO_NAO_ENCONTRADO",
            f"{item['CHAVE_MIGRACAO']}: {status}. {observacao}",
    ...
    pac.pesquisa["descartadas"] = descartadas


def _varredura_sintetica(pac: Pacote) -> None:
    """Texto com sinal de teste/exemplo: certo ⇒ sai; duvidoso ⇒ PENDENTE para validar."""
    grupos = [
        ...
        for registro in registros:
            ...
            if marcador:
                pac.pendencia(
                    ...
                    "Confirmar se o registro é real (mudar para OK) ou marcar NAO_IMPORTAR.", True,
                )
    if remover:
...
def _status(pac: Pacote) -> None:
    impede: dict[tuple[str, str], bool] = {}
    for p in pac.pendencias:
        chave = (p.arquivo, p.chave)
        impede[chave] = impede.get(chave, False) or p.impede_carga
    for arquivo, registros in (
        (CLIENTES, pac.clientes), (FORNECEDORES, pac.fornecedores), (PRODUTOS, pac.produtos), (OFERTAS, pac.ofertas),
    ):
        for r in registros:
            estado = impede.get((arquivo, r["CHAVE_MIGRACAO"]))
            r["STATUS_REVISAO"] = "PENDENTE" if estado else ("REVISAR" if estado is False else "OK")
    for r in pac.itens:
        estado = impede.get((r["_arquivo"], r["CHAVE_MIGRACAO"]))
        r["STATUS_REVISAO"] = "PENDENTE" if estado else ("REVISAR" if estado is False else "OK")

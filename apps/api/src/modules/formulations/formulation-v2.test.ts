...
async function createProduct(app: App, finishedItemId: string) {
  const response = await app.inject({
    method: "POST",
    url: "/products",
    payload: { customerId: await fixtureCustomerId(), name: `Produto F2 ${marker()}`, finishedProductItemId: finishedItemId },
  });
    ...
    const requirement = computeComponentRequirement(
      {
        ...
        // Quantidade TEÓRICA: os ajustes agora são autorizados, não
        // deduzidos da presença do campo.

...
async function createProduct(app: App, finishedItemId: string) {
  const product = (
    await app.inject({
      method: "POST",
      url: "/products",
      payload: { customerId: await fixtureCustomerId(), name: `Produto Relatórios ${marker()}`, finishedProductItemId: finishedItemId },
    })
  ).json();
...
describe("Relatórios — Comercial e Faturamento", () => {
  ...
  it("R-13: produzido vem das OPs do pedido, não do reservado nem do expedido", async () => {
    ...
    await aplicarRoteiroDeTeste(generated.id);

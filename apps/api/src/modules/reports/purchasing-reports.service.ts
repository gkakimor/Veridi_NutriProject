...
export async function getReceiptsReport(
  ...
  const [lines, total] = await Promise.all([
    prisma.receiptLine.findMany({
      ...
      include: {
        ...
        receipt: { include: { purchaseOrder: true, supplier: true, customer: true } },
      },
  ...
  const rows = lines.map((line): ReceiptReportRowDTO => ({
    ...
    purchaseOrderCode: line.receipt.purchaseOrder?.code ?? null,
    ...
    orderedUnitPrice: line.purchaseOrderLine?.unitPrice
      ? line.purchaseOrderLine.unitPrice.toString()
      : null,

 ...
 * `issuedAt`. O valor de cada linha e o do proprio documento
 * (`valorDoFaturamento`), e o total do periodo so existe quando TODOS os
 * documentos filtrados tem valor — soma parcial nunca e apresentada como total.
 */
export async function getBillingPeriodReport(
   ...
   * página pedida.
   *
   * O valor é o dos DOCUMENTOS, pela mesma conta do "Valor faturado" do Painel
   * (`resumirValorFaturado`, BILLED-VALUE-CANONICAL-01): `totalAmount`, com o
   * desconto apropriado e o ajuste de fechamento. A soma de `quantidade × preço`
   * por preço distinto ignorava os dois e não arredondava a linha. A contagem do
   * resumo é a mesma da paginação.
   */
  const [billings, resumo] = await Promise.all([
    ...
    resumirValorFaturado(prisma, where),
  ]);

  const rows = billings.map((billing): BillingPeriodRowDTO => {
    // O valor do próprio documento; sem valor, `null` — nunca soma parcial.
    const amount = valorDoFaturamento(billing);
    return {
  ...
  return {
    rows,
    ...pageMeta(pagination, resumo.billingCount),
    summary: {
      billingCount: resumo.billingCount,
      billingsWithCompletePricing: resumo.billingsWithCompletePricing,
      totalAmount: resumo.totalAmount,
    },
  };
}

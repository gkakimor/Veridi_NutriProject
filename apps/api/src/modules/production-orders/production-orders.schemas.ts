 ...
 * máximo que a planilha atual mostra: o número é escolha do usuário. Leitura
 * de inteiro decimal (API-INT-COERCION-REMAINING-01): `"1e1"` não é 10.
 */
const numberOfPartsSchema = inteiroDecimalSchema("Informe um número inteiro de partes").pipe(
  z.number().min(1, "A produção tem ao menos 1 parte").max(99, "Máximo de 99 partes"),
);

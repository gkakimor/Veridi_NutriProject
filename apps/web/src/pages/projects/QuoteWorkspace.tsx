...
import { PARAM_LINHA_DO_ORCAMENTO, rotaDoOrcamento } from "../../lib/rota-do-orcamento";
import { QuoteConditionsForm } from "./QuoteConditionsForm";
import { DuplicateQuoteDialog } from "./DuplicateQuoteDialog";
import { formatQuantity } from "../../lib/quantity";
import { formatQuoteDate as formatDate } from "./quote-display";

/**
 * A proposta de UMA versão de orçamento — o que a página própria do Orçamento
 * trabalha (QUOTE-WORKSPACE-NAVIGATION-01).
 *
 * Tudo aqui veio da seção de Orçamentos do Projeto, sem regra nova: linhas,
 * formação de preço, condições, envio, aceite, recusa, duplicação e Pedido. O
 * que mudou foi o lugar — a versão tem endereço, e quem escolhe qual versão
 * abrir é a URL, não um estado da ficha do Projeto.
 *
...
export function QuoteWorkspace({
  ...
  project: ProjectDTO;
  /** A versão aberta — quem decide qual é a rota da página, nunca um estado daqui. */
  ...
  /*
   * Quem volta da simulação de CMV volta para esta página — e para a linha de
   * onde saiu, que vem na URL. A volta explícita da página (`voltar`) também
   * vem nela, e acompanha quem duplica: a versão nova continua voltando para
   * onde a pessoa estava.
   */
  const navigate = useNavigate();
  const [params] = useSearchParams();
  const returningToLineId = params.get(PARAM_LINHA_DO_ORCAMENTO);
  ...
  // Outra versão descarta qualquer digitação pendente da anterior.
  useEffect(() => {
    setRascunhoDeLinha({});
  }, [open.id]);
  ...
  /**
   * Duplicar como nova versão — QUOTE-DUPLICATE-01.
   *
   * A versão nova é OUTRO documento: a tela vai para a página dela, com a mesma
   * volta que esta página oferecia. Recarregar esta versão antes seria uma
   * leitura que ninguém vai ver.
   */
  async function duplicar(origem: QuoteVersionDTO, priceStrategy: QuoteDuplicatePriceStrategy) {
    setSaving(true);
    setError(null);
    try {
      const criada = await duplicateQuoteVersion(origem.id, priceStrategy);
      navigate(rotaDoOrcamento(criada.id, { voltar }));
    } catch (err) {
      setError(apiErrorMessage(err, "Falha na operação"));
    } finally {
      setSaving(false);
    ...
    <div className="quote-workspace">
      ...
      <DuplicateQuoteDialog
        ...
        onConfirm={(priceStrategy) => {
          ...
          setDuplicando(null);
          if (!origem) return;
          void duplicar(origem, priceStrategy);
        }}
      />
    </div>
  );

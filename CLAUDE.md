...
- Migrations have three separate commands, and creating is not one of the
  other two:
  - **create** — `pnpm migration:create <name_in_snake_case>`. The only path
    that writes a migration. It does not apply it.
  - **apply** — `pnpm db:migrate`. Applies what already exists on the local
    database and can never create: it is `prisma migrate deploy`, which has no
    `--name`, and the wrapper rejects arguments before Prisma sees them.
  - **prove** — `pnpm validate:migrations:fresh`. Rebuilds a throwaway
    database from the chain and compares it to `schema.prisma`.

  Never call `prisma migrate dev` directly. The chain's tip is already ahead of
  the real clock, so Prisma's own timestamp sorts *before* migrations the new
  one depends on and breaks the rebuild from an empty database.
